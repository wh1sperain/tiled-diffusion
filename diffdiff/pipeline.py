...
from utils import wrap_edges_x, wrap_edges_y, pad_tensor_x
...
class StableDiffusionXLDiffImg2ImgPipeline(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin):
    ...
    def __call__(
        ...
        map = torchvision.transforms.Resize(tuple(s // self.vae_scale_factor for s in map.shape[1:]),
                                            antialias=None)(map)
        ...
        # Tiled Diffusion
        original_with_noise = pad_tensor_x(tensor=original_with_noise, max_width=max_width)
        original_with_noise = wrap_edges_x(tensor=original_with_noise, max_width=max_width)
        ...
        if not output_type == "latent":
            ...
            if max_width != 0:
                image = image[:, :, :,
                        self.vae_scale_factor * max_width: - self.vae_scale_factor * max_width]
        else:
            image = latents
            if max_width != 0:
